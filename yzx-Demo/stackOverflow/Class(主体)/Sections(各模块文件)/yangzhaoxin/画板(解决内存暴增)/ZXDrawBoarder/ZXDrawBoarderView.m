...
@interface ZXDrawBoarderView ()
{
    // 工具条的view
    UIView *_toolView;
    
    // 画板view
    ZXScrollView *_boardView;
}

@property (nonatomic, strong) NSArray *buttonImgNames;    // 按钮图片
@property (nonatomic, strong) NSArray *btnEnableImgNames;    // 按钮不可用图片

@property (nonatomic, strong) ZXMyDrawer *myDrawer;    // 画布

@property (nonatomic, strong) UIButton *delAllBtn;    // 删除按钮
@property (nonatomic, strong) UIButton *fwBtn;    // 上一步
@property (nonatomic, strong) UIButton *ntBtn;    // 下一步

@end

@implementation ZXDrawBoarderView

- (ZXMyDrawer *)myDrawer {
    if (!_myDrawer) {
        _myDrawer = [[ZXMyDrawer alloc] initWithFrame:CGRectMake(0, 0, kWindowW * 5, kWindowH * 2)];
        _myDrawer.layer.backgroundColor = clear_color.CGColor;
    }
    return _myDrawer;
}
...
{
    frame = CGRectMake(0, kWindowH, kWindowW, kWindowH);
    self = [super initWithFrame:frame];
    if (self) {
        //顶部工具
        UIView *toolV = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kWindowW, 64)];
        toolV.backgroundColor = green_color;
        ...
        // 画板view
        ZXScrollView * boardV = [[ZXScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(toolV.frame), kWindowW, kWindowH - CGRectGetMaxY(toolV.frame))];
